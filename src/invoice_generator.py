...
class InvoiceGenerator:
    ...
    def amount_in_words(self, n):
        m = {10000: 'Ten Thousand', 8000: 'Eight Thousand'}
    ...
    def generate_invoice(self, records, output_path, invoice_no, invoice_date):
        ...
        ref = first.reference_no or f"OW/CPR/{first.police_unit.replace(' ', '')}/{invoice_no}/2026"
        ...
        hdr.autofit = True
        self._set_cell(hdr.cell(0, 0), f"Invoice No: CPR/INV/2026/{invoice_no}", size=11, bold=True, align=WD_ALIGN_PARAGRAPH.LEFT)
        ...
        for i, r in enumerate(records, 1):
            ...
            self._set_cell(row[7], self.fmt_money(r.fee_per_day), size=9)
            self._set_cell(row[8], self.fmt_money(r.total_fee), size=9)
